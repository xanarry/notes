...
降低因为指令之间数据依赖而导致的CPU停顿。

### 内存访问重排序

优化对数据的访问，目的为使用尽可能少的IO次数读取尽可能多的数据。
...
同步方法指用synchronized关键词修饰的方法，这样的方法能够保证任意时刻，都只有唯一一个进程进入该方法体。
...
当某个代码块被synchronized修饰时，则称其为同步代码块，在修饰代码块时，必须为synchronized关键词提供一个对象用户获取锁与释放锁。

当同步代码块在实例方法中时，通常给synchronized传入this即可，让其使用当前对象上的锁。
...
当同步代码块在静态方法中时(没有当前对象)，我们可以给synchronized传入当前类的class，因为class本身也是类。
...
除了为synchronized传入当前类的class对象或者其当前实例，synchronized也可以被传入任何实例对象。但通常要求传递给synchronized关键词的对象要用final修饰。形如：`private final Object lock = new Object()`，因为在代码的执行过程当中，一旦锁的对象被修改，那么该代码块也就同步在了不同的锁上，从而没法保障原子性、有序性、可见性。最终使得数据在多个线程方法的情况下无法保证一致性和正确性。
...
读写锁是一种改进型排他锁，允许多个线程同时进行读操作，但仅允许一个线程进行写操作，存在读操作时不可写，反之亦然。申请读写锁之后，从锁中分别拿出读锁和写锁，给reader和writer使用。
...
3. **使用便捷性**：内部锁的申请与释放只能在一个方法上或者方法内部进行，简单易用，不存在锁泄露的问题；显示锁使用更加灵活，可以跨方法申请与释放锁，但过程需要程序员自己把控，使用不好存在锁泄露的可能性。
...
内存屏障(Memery Barrier/Fence)是被插入到两个指令序列之间使用的，其作用是禁止编译器、处理器对指令重排序，从而保证有序性。内存屏障在两个指令之间就如一个屏障一样隔绝两条指令。
...
内存屏障可分为：加载屏障（Load Barrier）和存储屏障（Store Barrier）。这两个屏障一起保证了数据在多处理器之间是可见的。

- 加载屏障(Load Barrier)：`StoreLoad`屏障可充当加载屏障，作用是刷新处理器缓存，即清空无效化队列，使处理器在读取共享变量时，先从主内存或其他处理器的高速缓存中读取相应变量，更新到自己的缓存中。
- 存储屏障(Story Barrier)：`StoreLoad`屏障可充当存储屏障，作用是冲刷处理器缓存，即将写缓冲器内容写入高速缓存中，使处理器对共享变量的更新写入高速缓存或者主内存中。

#### 按有序性保障划分

内存屏障分为：获取屏障（Acquire Barrier）和释放屏障（Release Barrier）。 这两个屏障一起保证了临界区中的任何读写操作不可能被重排序到临界区之外。

- 获取屏障(Aquire Barrier)：相当于`LoadLoad屏障`与`LoadStore屏障`的组合。在读操作后插入，禁止该读操作与其后的任何读写操作发生重排序。
...
- CAS仅仅保障共享变量更新操作的原子性，它并不保障可见性，所以**使用CAS的变量需要使用volatile修饰**。
...
1. **线程只有在持有一个对象内部锁的情况下，才能够调用这个对象的wait()方法**，也就是synchronize中的对象与wait所属的对象必须为同一对象，在模板代码中表现为第一行的someObject与第三行的someObject必须相同，否则虚拟机会报`java.lang.IllegalMonitorStateException`异常。Object.wait()总是在临界区以内。**只有持有内部锁的时候才能调用wait/notify，使用lock是不行的**。
...
public void wait() {
    ...
    if (当前对象不在等待集中)
        addWaitSet(Thread.concurrentThread());//将当前线程加入等待集
...
1. **当且仅当线程在持有一个对象内部锁的情况下才能够调用该对象的notify方法**。
2. 由于”线程只有在持有一个对象内部锁的情况下才能够调用该对象的notify方法“，所以要求Object.wait()暂停其执行线程的同时，必须释放相应的内部锁，否则线程无法进入临界区执行通知操作。
...
- 信号丢失：线程执行wait方法时，如果没有对共享条件进行循环检测，在通知线程已经发出了通知且共享条件已经满足的情况下仍然执行了wait方法。这种情况下线程将无休止等待，知道收到下一次通知。对于这种场景，只要将对保护条件的判读和wait方法放在一个循环语句中就可以避免信号丢失的问题。
...
## CyclicBarrier/栅栏（先到就等，到齐再走）

如果在代码中有这样一种需求：多个线程或者任务约定一个集合点，先到达该集合点的线程或者任务必须等待没有到达的线程或其他任务，只有当所有线程或任务都到达集合点之后，大家才能继续向后运行处理后续任务。那么使用CyclicBarrier能很好的解决这个问题，**Cyclic也就表示CyclicBarrier的对象实例是可以反复使用的**。
...
线程的中断机智也相当于是线程间的一种通信方式，当一个线程给另一个线程发送中断信息时，表示希望收到信息的线程能够停止其正在执行的工作，但**收到信号的线程采取什么样的行动有该线程自己决定，可能停止当前任务，也可能忽略该信号**。
...
同时考虑到线程在执行任务的时候可能会被阻塞或其中断标志变量可能会被清空。比较通用的方式是同时使用中断和表示变量：**先设置标志变量，然后给目标线程发送中断，线程在检查停止标志的时候也应该先检查条件变量，然后检查中断标志**
...
java使用ThreadLocal<T>来表示线程局部变量，每个ThreadLocal为每个线程只能持有一个T类型的数据，但可以为多个线程使用，即为多个线程分别持有一个类型为T的特有数据。因此，如果希望ThreadLocal能存储更多或者更复杂的数据，我们需要使用多个ThreadLocal，或者自定义一个类，封装需要local化的数据。



### 使用线程局部变量的优势

对于多个线程的共享遍历：避免多个线程竞争共享变量产生的上下文开销，提升了并发能力。

对个单个线程使用的变量：线程方法通常会被不止一次的调用，如果使用局部变量的形式，会导致变量被频繁的创建与销毁，而ThreadLocal可以避免这种情况。
